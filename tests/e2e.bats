...
EOF
  ...
  # Inspect the bbolt database in a retry loop to verify UnprepareResourceClaims is called
  # and pod configs have been deleted.
  local retries=10
  local wait_sec=2
  local removed=false
  for ((i=1; i<=retries; i++)); do
    # Try streaming the database file using cat and redirecting it to the local test path.
    if ! docker exec "$NODE_NAME" cat /var/run/dranet/dranet.db > "$BATS_TEST_DIRNAME/../_artifacts/dranet.db"; then
      sleep $wait_sec
      continue
    fi
    # Try reading the keys for the deleted pod using bbolt CLI.
